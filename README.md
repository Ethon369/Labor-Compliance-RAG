...
面向中国劳动法体系的 **检索增强生成（RAG）问答系统**。输入一个劳动争议场景问题（如“公司拖欠加班费怎么维权”），系统从《劳动法》《劳动合同法》原文中检索相关条款，给出**带准确条文引用**的回答。

> **定位**：面向中国劳动法体系的辅助参考工具，**不是法律咨询系统**——回答不构成任何法律意见。
...
在 50 问评测集（Top-8）上，使用 SiliconFlow 真模型（bge-m3 + bge-reranker-v2-m3）的基线成绩：

| 检索方法 | Recall@8 | HitRate@8 |
| :--- | :---: | :---: |
| 纯 BM25 | 0.789 | 0.830 |
| 纯向量 (bge-m3) | **0.969** | **1.000** |
| 混合 (RRF) | 0.928 | 0.962 |
| **混合 + Rerank** | **0.965** | **0.981** |

> **结论**：真模型下语义向量召回显著优于 BM25，RRF 融合两者互补，rerank 精排进一步提升排序质量。
