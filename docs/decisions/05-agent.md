...
| 价值 | "会用框架" | "理解编排的 why：什么时候需要分支、状态怎么设计" |
...
"要不要拒答"这条边界必须确定、可测、离线可复现。用 LLM 判的坑：同一输入连续两次可能给出不同答案；新模型上线阈值会飘；测试断言不可靠。规则简单但诚实——top-1 条文对查询的意图词覆盖率 ≥ threshold → 可答，否则走拒答。threshold 是接口参数，验证/CI/演示三场景都能稳定触发两条支路。
...
LangGraph 的循环是在 conditional_edges 的 `path_map` 里把某个分支指回上游节点（如 `"rewrite"`），不是另外的 API。这和条件分叉用的是同一个机制——这也是"Graph 比 Chain 强在哪"的第二句话。
