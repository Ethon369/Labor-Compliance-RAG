...
- 面试/演示场景下，`curl --no-buffer` 就能看效果，不需要额外工具——与项目"离线可演示"的思路一致。
...
- **存储**：内存列表，最大 200 条。服务重启后清空——个人展示项目不需要持久化，面试演示场景重启丢失是可接受的
...
3. **历史存内存，不持久化**。演示/面试场景足够；生产环境需替换为数据库存储。换存储不改接口——GET /chat/history 返回格式不变。
